...
def generate_key_encrypt_file(source_file: Path | str, output_file: Path | str | None):
    key = generate_key()
    encrypt_file(key, source_file, output_file)
    print(f"Encrypted file: {output_file}")
    print(f"Keep the key safe: {key}")
...
def encrypt_file(key: str, source_file: Path | str, output_file: Path | str):
...
def decrypt_file(key: str, data: bytes) -> str | None:
    # In case the key provided is not 32 bits or is malformed,
    # which Fernet skips, resulting in timing difference
    key_hash = hashlib.sha256(key.encode()).digest()
    work_key = base64.urlsafe_b64encode(key_hash)
    try:
        return Fernet(work_key).decrypt(data).decode()
    except (InvalidToken, ValueError):
        return None
    except Exception as e:
        logger.info(f"Unexpected decryption error: {type(e).__name__}")


def _dummy_decrypt(data: bytes):
